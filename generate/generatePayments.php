...
if ($rows > 0) {
    ...
    $stmt = $pdo->query("SELECT id_order FROM orders where id_order NOT IN (SELECT order_id_order FROM payments) ORDER BY RAND() LIMIT $rows");
    $orders = $stmt->fetchAll(PDO::FETCH_ASSOC);

    foreach ($orders as $orderId) {
        $payment_method = randomPaymentMethod();

        $stmtInsert = $pdo->prepare("INSERT INTO payments (payment_method, order_id_order) VALUES (?, ?)");
